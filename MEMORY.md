...
## 2026-07-13 — Real company logo (mark + favicon) + main/legal-pages branch split

- **Fact (Gergő):** real logo delivered in `pragma logo/` (committed): "vilagos" = navy line-art labyrinth-P (11.5KB, clean), "sotet" = filled disc variant (176KB, messy CorelDRAW export with stray off-canvas paths — don't inline it anywhere). Both had the 8.5×11in page viewBox; drawing bounds found by rasterizing on transparency (getBBox lies — hidden geometry far off-page).
- **Decided:** header/footer mark = `public/pragma-mark.svg` (vilagos cropped to `viewBox="2710 3110 3075 3045"`), painted via CSS mask (`.logo-mark` in globals.css): `#2a3141` (logo navy) light / `#ecedf2` dark — ONE cached file serves both themes, no HTML bloat, no double-download. Wordmark stays the "Pragma" text next to the mark.
- **Decided:** favicon/app icons from the disc variant (reads as a coin at 16px): `src/app/favicon.ico` (16/32/48), `icon.png` (256), `apple-icon.png` (180, opaque #f4f1e9 bg). Old placeholder enso mark deleted from logo.tsx.
- **IMPORTANT (branch state):** the legal pages commit (75dab73, privacy + impressum with `[[PLACEHOLDER]]` identity fields) was moved OFF main to the **`legal-pages` branch** so the logo could ship without publishing placeholders (Gergő paused legalities). The Analytics-only-on-Vercel fix was re-applied to main separately. When Gergő supplies IČO/address/legal name: merge `legal-pages` into main, fill placeholders, re-run checks, push.
